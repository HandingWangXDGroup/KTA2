...
while Fes < maxFes
    for i = 1:problem.M
        dmodel     = dacefit(Trianing_data.dec,Trianing_data.obj(:,i),'regpoly0','corrgauss',THETA_S(i,:),1e-5.*ones(1,problem.D),100.*ones(1,problem.D));
        Model_sensitive{i}   = dmodel;
        THETA_S(i,:) = dmodel.theta;
    end
    Centers = zeros(problem.M,2);
    for i = 1:problem.M
        [~,N1] = sort(Trianing_data.obj(:,i));
        num = ceil(size(Trianing_data.dec,1).*tau);
        mean_index{1} = N1(1:num);
        mean_index{2} = N1(end-num:end);
        for j = 1:2
            Centers(i,j) = mean(Trianing_data.obj(mean_index{j},i));  % lambda and miu
        end
        for j = 1:2
            train_X = Trianing_data.dec(mean_index{j},:);
            train_Y = Trianing_data.obj(mean_index{j},i);
    ...
    for i = 1:size(PopNew,1)
        dist2 = pdist2(real( PopNew(i,:)),real(Trianing_data.dec));
        if min(dist2) > 1e-5
    ...
    if ~isempty(Offspring02)
        ...
        Trianing_data.dec = [Trianing_data.dec;Offspring.dec];
        Trianing_data.obj = [Trianing_data.obj;Offspring.obj];
